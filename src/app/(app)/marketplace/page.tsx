import type { Metadata } from "next";
import { auth } from "@/lib/auth";
import { db } from "@/lib/db";
import { MarketplaceClient } from "@/components/marketplace/marketplace-client";

export const metadata: Metadata = { title: "Loja de Packs" };

async function getData(userId: string) {
  const [packs, purchases] = await Promise.all([
    db.achievementPack.findMany({
      where: { isActive: true, isBase: false },
      orderBy: { sortOrder: "asc" },
      include: {
        achievements: { where: { isActive: true }, select: { id: true, name: true, icon: true, rarity: true } },
      },
    }),
    db.purchase.findMany({
      where: { userId, status: "COMPLETED" },
      select: { packId: true },
    }),
  ]);

  const purchasedPackIds = new Set(purchases.map((p) => p.packId));
  return { packs, purchasedPackIds };
}

export default async function MarketplacePage() {
  const session = await auth();
  const userId = session!.user!.id as string;
  const { packs, purchasedPackIds } = await getData(userId);

  return <MarketplaceClient packs={packs} purchasedPackIds={purchasedPackIds} />;
}
